...
- **Settings:** center name, logo, address, phone, currency, billing rules, receipt printer and paper width, footer, warning thresholds, auto-end, long-session alert, low-stock alerts, theme (dark/light), language, and automatic daily backup (hour, folder, how many to keep). Also *Back up now* and *Restore* (with confirmation, a safety copy, and an automatic restart).
...
### Not in version 1
Customer accounts, loyalty, prepaid balances, online reservations, QR codes, remote monitoring and PC locking (see §46 of the brief). The layering above keeps room for them. The UI text is English only. Language is a setting, and the text is kept together so translations can be added.
